<?php
/**
 * @version    SVN $Id: edit.php 831 2012-12-21 16:47:14Z dhorsfall $
...
<form action="<?php echo htmlspecialchars(JFactory::getURI()->toString()); ?>" method="post" name="adminForm" id="adminForm" class="form-validate" enctype="multipart/form-data">
  <div id="hwd-container"> <a name="top" id="top"></a>
    ...
    <div class="media-header">
      <?php if ($this->isNew):?>
        <h2 class="media-group-title"><?php echo JText::_('COM_HWDMS_NEW_GROUP'); ?></h2>
      <?php else: ?>
        <h2 class="media-group-title"><?php echo JText::sprintf( 'COM_HWDMS_EDIT_GROUPX', $this->escape($this->item->title)); ?></h2>
          <ul class="media-category-ls">
            <li class="media-albums-items"> <a rel="{handler: 'iframe', size: {<?php echo $this->utilities->modalSize('large'); ?>}}" class="modal" href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&view=groupmedia&tmpl=component&group_id=' . $this->item->id); ?>" title="<?php echo JText::_('COM_HWDMS_MANAGE_MEDIA'); ?>"><?php echo JText::_('COM_HWDMS_MANAGE_MEDIA'); ?></a> </li>
            <li class="media-albums-items"> <a rel="{handler: 'iframe', size: {<?php echo $this->utilities->modalSize('large'); ?>}}" class="modal" href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&view=groupmembers&tmpl=component&group_id=' . $this->item->id); ?>" title="<?php echo JText::_('COM_HWDMS_MANAGE_MEMBERS'); ?>"><?php echo JText::_('COM_HWDMS_MANAGE_MEMBERS'); ?></a> </li>
          </ul>
      <?php endif; ?>
      <div class="clear"></div>
    </div>
    ...
    <fieldset>
      ...
      <div class="formelm-buttons">
        <button type="button" onclick="Joomla.submitbutton('groupform.save')">
          <?php echo JText::_('JSAVE') ?>
        </button>
        <button type="button" onclick="Joomla.submitbutton('groupform.cancel')">
    ...
    <fieldset>
      ...
      <?php if (!$this->isNew):?>
        <div style="float:right;">
          <img src="<?php echo JRoute::_(hwdMediaShareDownloads::thumbnail($this->item, 3)); ?>" border="0" alt="<?php echo $this->escape($this->item->title); ?>" style="max-width:200px;" />
        </div>
