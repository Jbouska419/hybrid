<?php
/**
 * @version    SVN $Id: edit.php 830 2012-12-21 16:46:58Z dhorsfall $
...
<form action="<?php echo htmlspecialchars(JFactory::getURI()->toString()); ?>" method="post" name="adminForm" id="adminForm" class="form-validate" enctype="multipart/form-data">
  <div id="hwd-container"> <a name="top" id="top"></a>
    ...
    <div class="media-header">
      <?php if ($this->isNew):?>
        <h2 class="media-album-title"><?php echo JText::_('COM_HWDMS_NEW_ALBUM'); ?></h2>
      <?php else: ?>
        <h2 class="media-album-title"><?php echo JText::sprintf( 'COM_HWDMS_EDIT_ALBUMX', $this->escape($this->item->title)); ?></h2>
        <ul class="media-category-ls">
          <li class="media-albums-items"> <a rel="{handler: 'iframe', size: {<?php echo $this->utilities->modalSize('large'); ?>}}" class="modal" href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&view=albummedia&tmpl=component&album_id=' . $this->item->id); ?>" title="<?php echo JText::_('COM_HWDMS_MANAGE_MEDIA'); ?>"><?php echo JText::_('COM_HWDMS_MANAGE_MEDIA'); ?></a> </li>
        </ul>
      <?php endif; ?>
      <div class="clear"></div>
    </div>
    ...
    <fieldset>
      ...
      <div class="formelm-buttons">
        <button type="button" onclick="Joomla.submitbutton('albumform.save')">
          <?php echo JText::_('JSAVE') ?>
        </button>
        <button type="button" onclick="Joomla.submitbutton('albumform.cancel')">
    ...
    <fieldset>
      ...
      <?php if (!$this->isNew):?>
        <div style="float:right;">
          <img src="<?php echo JRoute::_(hwdMediaShareDownloads::thumbnail($this->item, 2)); ?>" border="0" alt="<?php echo $this->escape($this->item->title); ?>" style="max-width:200px;" />
        </div> 
